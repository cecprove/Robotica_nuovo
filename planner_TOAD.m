function [xd,xdd,phi,phit]=planner_TOAD(punto, tempo, phi_in, t, circonferenza1, circonferenza2)
    ...
    phi_f1=phi_in.finale1; %phi finale per il primo tratto, ovvero in A
    phi_f2 = phi_in.finale2;
    ...
    elseif t <= tf %se ci troviamo ad un tempo inferiore di tf, dobbiamo pianificare
        ...
        %B_2phi = [phi_i; phi_f1; 0; 0]; %vettore dei termini noti per l'orientamento
        B_2phi = [0; abs(phi_i - phi_f1); 0; 0];
        ...
        phi_e = phi_i + (s_phi / norm(phi_f1 - phi_i))*(phi_f1 - phi_i); %è la phi desiderata
        phi_et = (sdot_phi / norm(phi_f1 - phi_i))*(phi_f1 - phi_i);
    ...
    elseif t >= tf2%altrimenti rimaniamo alla configurazione da cui siamo partiti ovvero nel punto B
        ...
        %B_1phi = [phi_f1; phi_f2; 0; 0]; %vettore termini noti per l'orientamento
        B_1phi = [0; abs(phi_f1 - phi_f2); 0; 0];
        ...
        phi_e = phi_f1 + (s_phi /norm(phi_f2 - phi_f1)) * (phi_f2 - phi_f1); %phi desiderata
        phi_et = (sdot_phi /norm(phi_f2 - phi_f1))*(phi_f2 - phi_f1);
